...
namespace intake{
  ...
  namespace automatic{
    ...
    namespace balls{
      ...
      bool get_overBallActual(){
        return overBallActual;
      }
      bool get_over(){
        return overBall;
      }
      ...
      void updateVars(){
        ...
        if(puncher.get_value()<puncBallTal){//if there is pysicaly a ball
          ...
          if(/*PuncherRuning*/false)  puncBall=false;//if the punc in running and there is not a ball physicaly present
            ...
            using namespace okapi::literals;
            if(puncherTimer.getDtFromMark()>0_ms) puncBall=false;
            // else if(puncherTimer.getDtFromMark()>100_ms)  comRum=true;
          }
        ...
        if(!get_overEnabled() && overTimer.getDtFromMark()>100_ms)  set_overEnabled(true);//timer
    ...
    void calcV(){
      ...
      if(get_enabled()){
        ...
        if(overMode){
          using namespace okapi::literals;
          set_v(vOver);
          if(balls::get_feedTop() || balls::get_feedBottom() || timer.getDtFromMark()>1.5_s) overMode=false;
        }
        else{
